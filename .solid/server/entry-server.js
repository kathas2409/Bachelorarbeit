...
rootData ? rootData.default : void 0;
...
function renderAsync$1(fn, options) {
  return () => async (event) => {
    {
      return await event.env.getStaticHTML?.("/index") ?? new Response("Not Found", { status: 404 });
    }
  };
}

const renderAsync = (fn, options) => composeMiddleware([apiRoutes, inlineServerFunctions, renderAsync$1()]);
